 ...
 * <ul>
 * <li>Direct body (e.g., {@code Observable<User>}) calls {@code onNext} with the deserialize body
...
public final class RxJavaCallAdapterFactory extends CallAdapter.Factory {
  ...
  @Override public CallAdapter<?> get(Type returnType, Annotation[] annotations, Retrofit retrofit) {
    ...
    if (!isCompletable && !(returnType instanceof ParameterizedType)) {
      ...
      throw new IllegalStateException(name + " return type must be parametrized"
  ...
  private CallAdapter<Observable<?>> getCallAdapter(Type returnType, Scheduler scheduler) {
    ...
    if (rawObservableType == Response.class) {
      if (!(observableType instanceof ParameterizedType)) {
          throw new IllegalStateException("Response must be parametrized"
    ...
    if (rawObservableType == Result.class) {
      if (!(observableType instanceof ParameterizedType)) {
          throw new IllegalStateException("Result must be parametrized"
  ...
  private static final class CallOnSubscribe<T> implements Observable.OnSubscribe<Response<T>> {
    ...
    @Override public void call(final Subscriber<? super Response<T>> subscriber) {
      ...
      // Wrap the call in a helper which handles both unsubscribe and back pressure.
